...
def fetch_answers_from_web():
    ...
    # Fetch all 11 pages of general questions
    for page_num in range(1, 12):
        ...
        try:
            ...
            # Find all questions with checkmarks
            # Pattern: number followed by question, then options with ✓ marking correct
            blocks = re.findall(r'(\d+)\.\s*\n(.*?)(?=\d+\.\s*\n|$)', content, re.DOTALL)

            for num, block in blocks:
                q_num = int(num)
                # Find the option marked with ✓
                option_lines = re.findall(r'(✓?)([^✓\n]+)', block)
                opt_idx = 0
                for marker, text in option_lines:
                    if marker == '✓':
                        answers[q_num] = text.strip()
                        break
    ...
    for web_code, state_code in state_codes_web.items():
        ...
        try:
            ...
            for num, block in blocks:
                ...
                for marker, text in option_lines:
                    if marker == '✓':
                        answers[(state_code, q_num)] = text.strip()
                        break
        except Exception as e:
