...
# Download Natural Earth 50m country boundaries for smooth borders
...
def coords_to_svg_path(geometry):
    ...
    # Simplify to keep SVG size reasonable but still look good
    geometry = geometry.simplify(0.02, preserve_topology=True)
    ...
    for poly in polys:
        # Skip tiny polygons (small islands)
        if poly.area < 0.01:
            continue
        # Get exterior ring
