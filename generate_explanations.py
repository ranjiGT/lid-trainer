...
def enhance_explanation(de, en, question, correct_clean, correct, options, num):
    ...
    # For "Welches/Welche" questions, make more informative
    if de.startswith("Die richtige Antwort ist:"):
        # Try to incorporate the question context
        if "?" in question:
            q_text = question.split("?")[0].strip()
            if q_text.lower().startswith("was "):
                de = f"{correct_clean}."
            elif q_text.lower().startswith("wer "):
                de = f"{correct_clean}."
            elif q_text.lower().startswith("wo "):
                de = f"{correct_clean}."
