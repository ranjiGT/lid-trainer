...
def parse_pdf_questions(pdf_path):
    ...
    for i, part in enumerate(parts[1:]):
        ...
        if state_name in state_map:
            ...
            # Fallback to substring match (longest name first to avoid Sachsen matching Sachsen-Anhalt)
            for name, code in sorted(state_map.items(), key=lambda x: len(x[0]), reverse=True):
                if state_name == name:
                    state_code = code
                    break
...
def reverse_gender_forms(text):
    """Reverse gender form pairs like 'A / B' -> 'B / A'."""
    # Match patterns like "der X / die Y" or "X / Y"
    parts = text.split(' / ')
    if len(parts) == 2:
    ...
    if len(parts) > 2:
        ...
        return ' / '.join(p.strip() for p in reversed_parts)
    # Also try with "/" without spaces
    parts = text.split('/')
    if len(parts) == 2:
        return parts[1].strip() + '/' + parts[0].strip()
    return text
...
def score_option_match(opt, answer_text):
    """Score how well an option matches the answer text."""
    opt_norm = normalize_text(opt)
    ans_norm = normalize_text(answer_text)
    ans_reversed = normalize_text(reverse_gender_forms(answer_text))

    best_score = 0

    for ans in [ans_norm, ans_reversed]:
        # Exact match
        if opt_norm == ans:
            return 1.0

        # Substring match
        if ans in opt_norm or opt_norm in ans:
            score = min(len(ans), len(opt_norm)) / max(len(ans), len(opt_norm), 1)
            best_score = max(best_score, score)

        # First N chars match
        if opt_norm[:25] == ans[:25] and len(ans) > 15:
            best_score = max(best_score, 0.7)

    # Word similarity (with both normal and reversed)
    sim = text_similarity(opt, answer_text)
    best_score = max(best_score, sim)
    sim2 = text_similarity(opt, reverse_gender_forms(answer_text))
    best_score = max(best_score, sim2)

    # Special case: short answers like "5 %", "16", numbers
    if len(ans_norm) <= 5:
        if ans_norm in opt_norm.split():
            best_score = max(best_score, 0.9)
        # Handle "5" matching "5" in option
        ans_clean = ans_norm.replace('%', '').replace(' ', '')
        opt_clean = opt_norm.replace('%', '').replace(' ', '')
    ...
    for q in questions:
        ...
        if answer_text:
            ...
            for i, opt in enumerate(q['options']):
                score = score_option_match(opt, answer_text)
                if score > best_score:
                    best_score = score
                    best_match = i
                if score >= 1.0:
            ...
            if best_match >= 0 and best_score >= 0.4:
...
if __name__ == '__main__':
    ...
    # Manual overrides for questions that can't be fuzzy-matched
    manual_overrides = {
        'general_72': 3,   # Friedrich Merz (2025 update, website still shows Olaf Scholz)
        'general_99': 0,   # Arbeitgeberinnen/Arbeitgeber und Arbeitnehmerinnen/Arbeitnehmer
        'general_121': 0,  # 5%-Hürde
        'HB_9': 2,         # Präsidentin/Präsident des Senates
    }
    for q in questions:
        if q['id'] in manual_overrides:
            q['correct'] = manual_overrides[q['id']]
            print(f"  Manual override: {q['id']} -> option {manual_overrides[q['id']]}")
